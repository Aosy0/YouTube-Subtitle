...
// @version      1.0.0
// @description
// YouTube字幕を改善：文単位表示、カスタマイズ可能なスタイル、日本語字幕優先、自動翻訳対応
// @author       You
...
// ============================================
// Trusted Types ポリシー（YouTube CSP対応）
// ============================================
let ysePolicy;
if (window.trustedTypes && trustedTypes.createPolicy) {
  try {
    ysePolicy =
        trustedTypes.createPolicy('yse-policy', {createHTML: (input) => input});
  } catch (e) {
    // ポリシーが既に存在する場合はフォールバック
    console.warn('[YSE] TrustedTypesポリシー作成に失敗:', e);
  }
}

/**
 * innerHTML代入用のヘルパー関数
 * TrustedTypesが有効な環境ではポリシーを経由してHTMLを生成
 * @param {Element} element - 対象要素
 * @param {string} html - 設定するHTML文字列
 */
function safeSetInnerHTML(element, html) {
  if (ysePolicy) {
    element.innerHTML = ysePolicy.createHTML(html);
  } else {
...
const Logger = {
  ...
  log(level, message, ...args) {
    if (level >= this.currentLevel) {
      ...
      const logEntry =
          {timestamp, level: levelName, message: message, args: args};
  ...
  downloadLogs() {
    ...
    a.href = url;
    a.download =
        `yse-logs-${new Date().toISOString().replace(/[:.]/g, '-')}.txt`;
...
const LogPanel = {
  ...
  createPanel() {
    ...
    // イベントリスナー
    this.panel.querySelector('#yse-log-close')
        .addEventListener('click', () => this.hide());
    this.panel.querySelector('#yse-log-clear')
        .addEventListener('click', () => this.clearLogs());
    this.panel.querySelector('#yse-log-download')
        .addEventListener('click', () => this.downloadLogs());
    this.panel.querySelector('#yse-log-refresh')
        .addEventListener('click', () => this.refresh());

    const autoRefreshCheckbox =
        this.panel.querySelector('#yse-log-autorefresh');
  ...
  refresh() {
    ...
    if (logs.length === 0) {
      safeSetInnerHTML(
          content,
          '<div style="color: #666; text-align: center; padding: 20px;">ログがありません</div>');
      return;
    }
        ...
        logs.map(log => {
              ...
              return `<div style="
                ...
                ">
                  ...
                  log.level}]</span>
                    <span>${this.escapeHtml(log.message)}${
                  this.escapeHtml(argsStr)}</span>
                </div>`;
            })
            .join(''));
  ...
  getSubtitleTracks() {
    try {
      ...
      const playerResponse = player.getPlayerResponse?.() ||
          ytInitialPlayerResponse || ytplayer?.config?.args?.player_response;
  ...
  setSubtitlesEnabled(enabled) {
    try {
      ...
      if (player && player.setOption) {
        player.setOption(
            'captions', 'track', enabled ? {} : {'languageCode': ''});
      }
    } catch (e) {
  ...
  autoSelectSubtitle() {
    ...
    if (fallbackTrack) {
      ...
      if (autoTranslate) {
        ...
        Logger.info(`フォールバック字幕から自動翻訳: ${
            fallbackTrack.languageCode} → ${preferredLang}`);
      } else {
        this.setSubtitleLanguage(fallbackTrack.languageCode);
        Logger.info(
            `フォールバック言語の字幕を選択: ${fallbackTrack.languageCode}`);
      }
      return;
    }
    ...
    if (autoTranslate && tracks.length > 0) {
      ...
      this.setAutoTranslation(preferredLang);
      Logger.info(
          `字幕を自動翻訳: ${firstTrack.languageCode} → ${preferredLang}`);
      return;
    }
...
const SubtitleEnhancer = {
  ...
  injectStyles() {
    const styles = `
                ...
                .yse-caption-text {
                    ...
                    letter-spacing: ${
        Settings.get('letterSpacing')}px !important;
                }
  ...
  waitForCaptions() {
    ...
    const checkInterval = setInterval(() => {
      ...
      // 複数のセレクタを試行
      const selectors = [
        '.caption-window', '.ytp-caption-window', '.ytp-caption-window-top',
      ...
      if (captionWindow) {
        ...
        clearInterval(checkInterval);
        console.warn(
            '[YSE] 字幕コンテナが見つかりませんでした（タイムアウト）');
      }

      // 30秒ごとにログ出力
      if (attempts % 60 === 0) {
        console.log(
            `[YSE] 字幕コンテナを検索中... (${attempts}/${maxAttempts})`);
      }
    }, 500);
  },
  ...
  setupCaptionObserver(captionWindow) {
    try {
      ...
      // 字幕テキストの変更を監視（デバウンス付き）
      this.observer = new MutationObserver(() => {
        try {
          // 自動生成字幕は1文字ずつ追加されるため、デバウンスで安定するまで待つ
      ...
      this.observer.observe(
          captionWindow, {childList: true, subtree: true, characterData: true});
  ...
  processCaptionText(captionWindow) {
    try {
      ...
      // 字幕テキスト要素を探す（複数のセレクタに対応）
      const captionSegments =
          captionWindow.querySelectorAll('.ytp-caption-segment');
      const captionsText = captionWindow.querySelector('.captions-text');

      let fullText = '';

      if (captionSegments.length > 0) {
        // ytp-caption-segment が見つかった場合（通常の字幕・自動生成字幕）
        captionSegments.forEach(segment => {
          fullText += segment.textContent || '';
        });
      } else if (captionsText) {
        // captions-text から取得
        const spans = captionsText.querySelectorAll('span');
        if (spans.length > 0) {
          spans.forEach(span => {
            fullText += span.textContent || '';
          });
        } else {
          fullText = captionsText.textContent || '';
        }
      } else {
        return;
      }
  ...
  processText(text) {
    try {
      ...
      if (sentenceEndRegex.test(text)) {
        ...
        // 文がまだ続いている：一定時間後にフラッシュ（自動生成字幕対応）
  ...
  displaySentence(text) {
    // 字幕ウィンドウを探す
    const captionWindows = document.querySelectorAll(
        '.caption-window, .ytp-caption-window-top, .ytp-caption-window-bottom');
    ...
    captionWindows.forEach(captionWindow => {
      ...
      if (segments.length > 1) {
        // 複数のsegmentがある場合：最初のsegmentにテキストをまとめ、残りを非表示
        segments[0].textContent = text;
      ...
      // スタイルを適用
      const textEl =
          captionWindow.querySelector('.captions-text, .ytp-caption-segment');
      this.applyCustomStyles(captionWindow, textEl);
    });
...
const YouTubeSettingsIntegration = {
  ...
  startMenuCheckInterval() {
    ...
    this.menuCheckInterval = setInterval(() => {
      ...
      if (settingsMenu &&
          !settingsMenu.querySelector('.yse-settings-menu-item')) {
  ...
  observeSettingsMenu() {
    ...
    this.settingsMenuObserver.observe(
  ...
  onSettingsMenuOpened(menuElement) {
    ...
    if (!menuContent) {
      ...
      if (menuItems.length > 0) {
        Logger.debug(
            `既存のメニュー項目が ${menuItems.length} 個見つかりました`);
...
const UIController = {
  ...
  openSettings() {
    ...
    // 設定パネルをdocument.bodyに追加（スタッキングコンテキスト問題を回避）
    this.panel = document.createElement('div');
  ...
  generateSettingsHTML() {
    ...
    return `
                ...
                <div style="max-height: 60vh; overflow-y: auto; padding-right: 8px;">
                    <div class="yse-setting-group">
                        ...
                        <input type="text" class="yse-setting-input" data-key="preferredLanguage" 
                               value="${
        Settings.get('preferredLanguage')}" placeholder="例: ja, en, ko">
                    ...
                    <div class="yse-setting-group">
                        ...
                        <input type="text" class="yse-setting-input" data-key="fallbackLanguage" 
                               value="${
        Settings.get('fallbackLanguage')}" placeholder="例: en">
        ...
        Settings.get('sentenceMode') ? 'checked' : ''}>
                    ...
                    <div class="yse-setting-group">
                        <label class="yse-setting-label">フォントサイズ: <span id="fontSize-value">${
        Settings.get('fontSize')}</span>px</label>
                        <input type="range" class="yse-setting-input" data-key="fontSize" 
                               value="${
        Settings.get('fontSize')}" min="12" max="48" style="width: 100%;">
                    </div>
                    ...
                    <div class="yse-setting-group">
                        ...
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <input type="color" class="yse-setting-input" data-key="fontColor" 
                                   value="${
        Settings.get(
            'fontColor')}" style="width: 60px; height: 36px; padding: 2px;">
                            <input type="text" class="yse-setting-input" data-key="fontColor" 
                                   value="${
        Settings.get('fontColor')}" style="flex: 1;">
                        </div>
                    </div>
                    ...
                    <div class="yse-setting-group">
                        ...
                        <input type="text" class="yse-setting-input" data-key="backgroundColor" 
                               value="${
        Settings.get('backgroundColor')}" placeholder="rgba(0, 0, 0, 0.75)">
                    ...
                    <div class="yse-setting-group">
                        ...
                        <select class="yse-setting-input" data-key="position">
                            <option value="bottom" ${
        pos === 'bottom' ? 'selected' : ''}>下部（デフォルト）</option>
                            <option value="top" ${
        pos === 'top' ? 'selected' : ''}>上部</option>
                            <option value="custom" ${
        pos === 'custom' ? 'selected' : ''}>カスタム位置</option>
                        </select>
                    </div>

                    <div class="yse-setting-group" id="customPositionGroup" style="display: ${
        pos === 'custom' ? 'block' : 'none'};">
                        <label class="yse-setting-label">下端からの距離: <span id="customPositionY-value">${
        Settings.get('customPositionY')}</span>%</label>
                        <input type="range" class="yse-setting-input" data-key="customPositionY" 
                               value="${
        Settings.get('customPositionY')}" min="0" max="50" style="width: 100%;">
                    </div>
                    ...
                    <div class="yse-setting-group">
                        ...
                        <input type="number" class="yse-setting-input" data-key="maxLines" 
                               value="${
        Settings.get('maxLines')}" min="1" max="5">
                    </div>
                    ...
                    <div class="yse-setting-group">
                        ...
                        <input type="number" class="yse-setting-input" data-key="lineHeight" 
                               value="${
        Settings.get('lineHeight')}" min="1" max="3" step="0.1">
                    </div>
                    ...
                    <div class="yse-setting-group">
                        ...
                        <input type="number" class="yse-setting-input" data-key="letterSpacing" 
                               value="${
        Settings.get('letterSpacing')}" min="0" max="5" step="0.5">
                    </div>
                </div>
                ...
                <div class="yse-debug-info">
                    <div>Version: ${
        CONFIG.VERSION} | Alt+S でショートカット</div>
  ...
  attachEventListeners() {
    // 閉じるボタン
    this.panel.querySelector('.yse-settings-close')
    ...
    // 入力フィールドのリアルタイム更新
    this.panel
        .querySelectorAll('.yse-setting-input, .yse-setting-checkbox input')
        .forEach(input => {
          // レンジスライダーのリアルタイム更新
          if (input.type === 'range') {
          ...
          input.addEventListener('change', (e) => {
            ...
            if (key === 'position') {
              const customGroup =
                  this.panel.querySelector('#customPositionGroup');
              if (customGroup) {
...
const DebugIndicator = {
  ...
  createIndicator() {
    ...
    // 安全なDOM APIを使用（innerHTML回避）
    const dot = document.createElement('span');
    dot.textContent = '●';
    dot.style.color = '#4ade80';

    const text = document.createElement('span');
    text.textContent = 'YSE';
    ...
    // 複数のセレクタを試行
    const selectors = [
      '#masthead-container #start', '#masthead #start', 'ytd-masthead #start',
